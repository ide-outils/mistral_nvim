...
use crate::internals::{
    ...
    attr::{Default, Identifier, TagType},
    ungroup,
};
                ...
                if let Default::None = field.attrs.default() {
                    if let Some(first) = first_default_index {
                        cx.error_spanned_by(
                            field.ty,
                            format!(
                                "field must have #[serde(default)] because previous field {} has #[serde(default)]",
                                first
                            ),
                        );
                    }
    ...
    match cont.data {
        Data::Enum(_) => {
            if cont.data.has_getter() {
                cx.error_spanned_by(cont.original, "#[serde(getter = \"...\")] is not allowed in an enum");
            }
        }
        Data::Struct(_, _) => {
...
fn check_flatten_field(cx: &Ctxt, style: Style, field: &Field) {
    ...
    match style {
        Style::Tuple => {
            cx.error_spanned_by(field.original, "#[serde(flatten)] cannot be used on tuple structs");
        }
        Style::Newtype => {
            cx.error_spanned_by(field.original, "#[serde(flatten)] cannot be used on newtype structs");
        }
        _ => {}
    }
}
    ...
    for (i, variant) in variants.iter().enumerate() {
        match (
            ...
            (_, Identifier::No, true, &TagType::None) => {
                cx.error_spanned_by(variant.original, "#[serde(other)] cannot appear on untagged enum");
            }

            // Variant with `other` attribute must be the last one.
            (Style::Unit, Identifier::Field, true, _) | (Style::Unit, Identifier::No, true, _) => {
                if i < variants.len() - 1 {
                    cx.error_spanned_by(variant.original, "#[serde(other)] must be on the last variant");
                }
            }
    ...
    for variant in variants {
        match variant.style {
            Style::Struct => {
                ...
                for field in &variant.fields {
                    let check_ser = !(field.attrs.skip_serializing() || variant.attrs.skip_serializing());
                    let check_de = !(field.attrs.skip_deserializing() || variant.attrs.skip_deserializing());
...
fn check_transparent(cx: &Ctxt, cont: &mut Container, derive: Derive) {
    ...
    let fields = match &mut cont.data {
        ...
        Data::Struct(Style::Unit, _) => {
            cx.error_spanned_by(cont.original, "#[serde(transparent)] is not allowed on a unit struct");
            return;
        }
