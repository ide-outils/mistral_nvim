...
use quote::ToTokens;
use syn::{
    Ident, Lifetime, Token, meta::ParseNestedMeta, parse::ParseStream, punctuated::Punctuated, spanned::Spanned, token,
};
...
impl Container {
    ...
    pub fn from_ast(cx: &Ctxt, item: &syn::DeriveInput) -> Self {
        ...
        for attr in &item.attrs {
            if attr.path() != SERDE {
                non_exhaustive |= matches!(&attr.meta, syn::Meta::Path(path) if path == NON_EXHAUSTIVE);
                continue;
            }
                    ...
                    match &variant.fields {
                        ...
                        syn::Fields::Unnamed(fields) => {
                            if fields.unnamed.len() != 1 {
                                let msg = "#[serde(tag = \"...\")] cannot be used with tuple variants";
...
fn decide_identifier(
    ...
    match (
        ...
        (_, Some((field_identifier_tokens, ())), Some((variant_identifier_tokens, ()))) => {
            let msg = "#[serde(field_identifier)] and #[serde(variant_identifier)] cannot both be set";
...
impl Variant {
    pub fn from_ast(cx: &Ctxt, variant: &syn::Variant) -> Self {
            ...
            if let Err(err) = attr.parse_nested_meta(|meta| {
                if meta.path == RENAME {
                    ...
                    let path = meta
                        ...
                        .to_string()
                        .replace(' ', "");
                    return Err(meta.error(format_args!("unknown serde variant attribute `{}`", path)));
                }
                Ok(())
            }) {
                cx.syn_error(err);
        ...
        Variant {
            name: MultiName::from_attrs(Name::from(&unraw(&variant.ident)), ser_name, de_name, Some(de_aliases)),
            rename_all_rules: RenameAllRules {
                serialize: rename_all_ser_rule.get().unwrap_or(RenameRule::None),
                deserialize: rename_all_de_rule.get().unwrap_or(RenameRule::None),
            },
    ...
    pub fn rename_by_rules(&mut self, rules: RenameAllRules) {
        if !self.name.serialize_renamed {
            self.name.serialize.value = rules
                .serialize
                .apply_to_variant(&self.name.serialize.value);
        }
...
impl Field {
    ...
    pub fn rename_by_rules(&mut self, rules: RenameAllRules) {
        if !self.name.serialize_renamed {
            self.name.serialize.value = rules
                .serialize
                .apply_to_field(&self.name.serialize.value);
        }
        if !self.name.deserialize_renamed {
            self.name.deserialize.value = rules
                .deserialize
...
fn get_renames(cx: &Ctxt, attr_name: Symbol, meta: &ParseNestedMeta) -> syn::Result<SerAndDe<syn::LitStr>> {
    let (ser, de) = get_ser_and_de(cx, attr_name, meta, get_lit_str2)?;
    Ok((ser.at_most_one(), de.at_most_one()))
}

fn get_multiple_renames(cx: &Ctxt, meta: &ParseNestedMeta) -> syn::Result<(Option<syn::LitStr>, Vec<syn::LitStr>)> {
    let (ser, de) = get_ser_and_de(cx, RENAME, meta, get_lit_str2)?;
    Ok((ser.at_most_one(), de.get()))
}

fn get_where_predicates(cx: &Ctxt, meta: &ParseNestedMeta) -> syn::Result<SerAndDe<Vec<syn::WherePredicate>>> {
    let (ser, de) = get_ser_and_de(cx, BOUND, meta, parse_lit_into_where)?;
    Ok((ser.at_most_one(), de.at_most_one()))
}

fn get_lit_str(cx: &Ctxt, attr_name: Symbol, meta: &ParseNestedMeta) -> syn::Result<Option<syn::LitStr>> {
...
fn parse_lit_into_path(cx: &Ctxt, attr_name: Symbol, meta: &ParseNestedMeta) -> syn::Result<Option<syn::Path>> {
...
fn parse_lit_into_ty(cx: &Ctxt, attr_name: Symbol, meta: &ParseNestedMeta) -> syn::Result<Option<syn::Type>> {
    let string = match get_lit_str(cx, attr_name, meta)? {
        Some(string) => string,
        None => return Ok(None),
    };
...
fn parse_lit_into_lifetimes(cx: &Ctxt, meta: &ParseNestedMeta) -> syn::Result<BTreeSet<syn::Lifetime>> {
...
fn borrowable_lifetimes(cx: &Ctxt, name: &Name, field: &syn::Field) -> Result<BTreeSet<syn::Lifetime>, ()> {
