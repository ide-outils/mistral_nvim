...
use super::{Ctxt, Derive, attr, check};
...
impl<'a> Container<'a> {
    ...
    pub fn from_ast(cx: &Ctxt, item: &'a syn::DeriveInput, derive: Derive, private: &Ident) -> Option<Container<'a>> {
        let attrs = attr::Container::from_ast(cx, item);

        let mut data = match &item.data {
            syn::Data::Enum(data) => Data::Enum(enum_from_ast(cx, &data.variants, attrs.default(), private)),
            syn::Data::Struct(data) => {
                let (style, fields) = struct_from_ast(cx, &data.fields, None, attrs.default(), private);
...
fn enum_from_ast<'a>(
    ...
    let variants: Vec<Variant> = variants
        ...
        .map(|variant| {
            ...
            let (style, fields) = struct_from_ast(cx, &variant.fields, Some(&attrs), container_default, private);
            Variant {
